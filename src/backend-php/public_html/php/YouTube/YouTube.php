...
function echoVideoList($feed)
{
    ...
    foreach ($feed as $entry) {
        ...
        $videoTitle = $entry->mediaGroup->title;
        $videoDescription = substr($entry->mediaGroup->description, 0, 300);
        print <<<END
        <tr>
        <td width="100"><img src="${thumbnailUrl}" width='100'/></td>
        <td width="100%">
        <a href='#' onClick="validateForm('$videoId');">${videoTitle}</a>
